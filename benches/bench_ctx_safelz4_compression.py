...
def compression_write_safelz4(file: tempfile.NamedTemporaryFile, chunks: Generator[bytes, None, None]):
    with safelz4.open(file.name, "wb") as output:
        for chunk in chunks:
            output.write(chunk)
...
def main():
    for name in AVAILABLE_FILES:
        benchmark_compression(name, compression_write_safelz4)
