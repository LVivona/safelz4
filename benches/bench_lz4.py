...
def bench_func(name: str):
    ...
    py_runner.bench_func(f"compress_{name}", lz4.frame.compress, *[buffer])
    output = lz4.frame.compress(buffer)
    py_runner.bench_func(f"decompress_{name}", lz4.frame.decompress, *[output])
    del output
    del buffer
